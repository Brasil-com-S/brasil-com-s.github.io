// Brasil Com S - Samba Schools Catalog & Interactivity
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderSchools(list) {
    ...
    list.forEach(school => {
      ...
      card.innerHTML = `
        <img src="${school.logo_url}" alt="Brasão ${school.nome}" class="school-logo" />
        ...
        <button class="btn-details" data-slug="${school.slug}">Ver Histórico Completo</button>
      `;
  ...
  function applyFilters() {
    ...
    const filtered = allEscolas.filter(school => {
  ...
  function openSchoolModal(school) {
    ...
    const sambasHtml = school.sambas_enredo ? school.sambas_enredo.map(s => `
      <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); padding: 0.8rem 1rem; border-radius: 8px; margin-bottom: 0.6rem;">
        ...
        <div style="font-size: 0.82rem; color: #94A3B8; margin-top: 0.2rem;">Compositores: ${s.compositores}</div>
      </div>
    ...
    const colocacoesHtml = school.colocacoes ? school.colocacoes.map(c => `
      <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
        ...
        <td style="padding: 0.6rem; font-weight: 700;">${c.posicao}º Lugar</td>
        <td style="padding: 0.6rem; color: #94A3B8;">${c.pontos} pts</td>
      </tr>
    ...
    modalBody.innerHTML = `
      <div style="display: flex; align-items: center; gap: 1.5rem; margin-bottom: 2rem; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 1.5rem;">
        <img src="${school.logo_url}" alt="${school.nome}" style="width: 110px; height: 110px; border-radius: 50%; border: 3px solid #FFCC00; object-fit: cover;" />
        <div>
      ...
      <div style="margin-bottom: 2rem;">
        <h3 style="font-family: 'Outfit', sans-serif; font-size: 1.3rem; margin-bottom: 1rem; color: #FFCC00;">🎵 Sambas-Enredo Históricos</h3>
        ${sambasHtml}
      </div>
        ...
        <table style="width: 100%; text-align: left; border-collapse: collapse; font-size: 0.9rem;">
          <thead>
            <tr style="border-bottom: 1px solid rgba(255,255,255,0.1); color: #94A3B8;">
              ...
              <th style="padding: 0.6rem;">Posição</th>
              <th style="padding: 0.6rem;">Pontuação</th>
            </tr>
          </thead>
          <tbody>
            ${colocacoesHtml}
